...
class Models(WMLResource):
                ...
                if response.status_code == 400 and (
                    ...
                    if spec_lifecycle.get("retired"):
                        if retired_since := spec_lifecycle.get("retired_since_version"):
                            warnings.warn(
                                f"Software specification `{sw_spec}` is retired since version {retired_since}.{replacement_str}"
                            )
                        else:
                            warnings.warn(
                                f"Software specification `{sw_spec}` is retired.{replacement_str}"
                            )
                        ...
                        if deprecated_since := spec_lifecycle.get(
                            ...
                            warnings.warn(
                                f"Software specification `{sw_spec}` is deprecated since version {deprecated_since}.{replacement_str}"
                            )
                        else:
                            warnings.warn(
                                f"Software specification `{sw_spec}` is deprecated.{replacement_str}"
                            )
    ...
    def store(
        ...
        **Examples**
        ...
        if model is None:
            ...
            saved_model = self._store_curated_foundation_model(model, meta_props)
    ...
    def list_revisions(
        ...
        **Example:**
        ...
        model_resources = self._get_artifact_details(
            url, "revisions", limit, "model revisions", _all=get_all
        )["resources"]
    ...
    def _store_curated_foundation_model(
        self, model: str, meta_props: dict[str, Any]
    ) -> dict[str, Any]:
        # Store curated foundation model

        payload = self._create_curated_model_payload(model=model, meta_props=meta_props)
    ...
    def _create_curated_model_payload(
        self, model: str, meta_props: dict[str, Any]
    ) -> dict[str, Any]:
        metadata = copy.deepcopy(meta_props)
